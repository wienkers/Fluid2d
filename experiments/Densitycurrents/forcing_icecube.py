...
class Forcing(Param):
    ...
    def __init__(self,param,grid):
        ...
        type_forcing = 'icecube'#'horizontalconvection'

        # type_forcing = 'prescribed_buoyancy'

        if type_forcing=='icecube':
            x = grid.xr / grid.Lx
            y = 1.-grid.yr / grid.Ly
            if grid.j0 == grid.npy-1:
                self.forc[:, :] = -Q * np.exp(-(x-0.5)**2/(2*sigma**2) -y*20.)
                print(self.forc[-3:,30:40])
    ...
    def add_forcing(self, x, t, dxdt):
        ...
        if self.type_forcing == 'prescribed_buoyancy':
            ...
            if self.j0 == self.npy-1:
                dxdt[4][-self.nh-1,:] -= (x[4][-self.nh-1,:]+ self.deltab*.5)/self.dt
            if self.j0 == 0:
                dxdt[4][self.nh,:] -= (x[4][self.nh,:]-self.deltab*.5)/self.dt
